...
subroutine genflu_2(ut,vt,wt,y,re,dt,tiempo,mpiid,m,communicator)
  !------------------------------------------------------------------------*
  ...
  use temporal_2
  use point_2
  use alloc_dns_2,only:kazr,idx,inbx,dy,idy,v0
  use main_val_2, only:wkp
  use ctesp_2
  use omp_lib
  implicit none
  include "mpif.h"
  ...
  if (mpiid==0) then   
	...
	write(*,*) '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'
     ...
     do k=1,2
        ...
        call dobleu_2(ysca,wlwa,www1,ny) 
     ...
     call profile_2(ut,wkp,ntop(1),kl(1,1),fsca(1,1),ny+1)
     call profile_2(wt,wkp,ntop(1),kl(1,1),fsca(1,1),ny+1)     
     call profile_2(vt,wkp,ntop(2),kl(1,2),fsca(1,2),ny  )
  endif

#ifndef NOINFOSTEP
  !Writting info per time substep at different X locations
  call info_per_step_2(ut,y,tiempo,dt,re,ntop,jtop,mpiid) 
#endif
...
end subroutine genflu_2

! ---------------------------------------------------------------------- !
! ------------------------ Auxiliary functions ------------------------- !
! ---------------------------------------------------------------------- !

subroutine dobleu_2(eta,www,www1,n)
  implicit none
  ...
  enddo
end subroutine dobleu_2
!--------------------------------------------------------
!--------------------------------------------------------
!--------------------------------------------------------

subroutine profile_2(ut,wk,ntop,kl,fsca,n)
  use ctesp_2
  use genmod_2  
  implicit none
...
!== COPY BACK====    
  ...
  enddo
end subroutine profile_2

!------------------------------------------------------
!-------------TIME PER SUBSTEP INFO---------------------
!------------------------------------------------------
subroutine info_per_step_2(ut,y,tiempo,dt,re,ntop,jtop1,mpiid,communicator) 
  use genmod_2
  use point_2
  use ctesp_2
  use omp_lib
  use alloc_dns_2,only:dy
  implicit none
  include "mpif.h"
  integer,intent(in)::communicator
...
subroutine genflu_2(ut,vt,wt,y,re,dt,tiempo,mpiid,m,communicator)
  ...
  if (mpiid==0) then   
     ...
     wlws=y/din
     call dobleu_2(wlws,wlws,ny+2) ! weigth function of Lundt wu and squires
     tm3 = MPI_WTIME()
     ! -------  the zero mode ---------
     ut(0,:,1) = u0
     ut(1,:,1) = 0d0
     ...
     call profile_2(ut,wkp,wkpo,ntop,kl,fsca,wlws,ny+1)
     call profile_2(vt,wkp,wkpo,ntop,kl,fsca,wlws,ny  )
     call profile_2(wt,wkp,wkpo,ntop,kl,fsca,wlws,ny+1)     
  endif
...
subroutine profile_2(ut,wk,wko,ntop,kl,fsca,wlws,n)
