...
subroutine statsp_2(u_x,u,v,w,p, &
  ...
  do i=ib,ie 
     call interp_pressure_2(p(0,1,i),buf5(0,1),ny) !buf5=p(...,i) [outside parallel region!]
...
!----------------------------------------------------------
  ...
  do i=ib,ie
     call interp_pressure_2(p(0,1,i),buf1(0,1),ny) !buf1=p(...,i) [outside parallel region!]
     !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(j,k,cte)
     !$OMP DO SCHEDULE(STATIC)
     do k = 0,nz2
