...
subroutine statsp_2(u_x,u,v,w,p, &
  ...
  do i=ib,ie 
     ...
     enddo
     !call interpyy_2(buf5(0,1),buf5(0,1),inyu,cofiuy,inby,ny+1,0,nz1,nz1) !buf5=p_y  !works in place
          
     !================Statistics for Reynolds stresses: uu,vv,ww and uv    
     call interpyy_2(u_x(0,1,i),buf1(0,1)  ,inyu,cofiuy,inby,ny+1,0,nz1,nz1)   !buf1=(u_x)_y          
     call interpyy_2(w(0,1,i)   ,buf2(0,1)  ,inyu,cofiuy,inby,ny+1,0,nz1,nz1)   !buf2=w_y 
     ...
     call interpyy_2(dwdx(0,1,i),dwdx(0,1,i),inyu,cofiuy,inby,ny+1,0,nz1,nz1)  !after interpyy: dwdx==[d(w_x)/dx]_y!!  
     ...
     call differzy_2(u_x(0,1,i),buf1(0,1),dcyv,dcby,cofcyv,1,1,0,1,ny+1) !buf1=dudy=d(u_x)dy
     !$OMP DO SCHEDULE(STATIC)
     do j=1,ny+1        
     ...
     call interpyy_2(buf2(0,1),buf2(0,1),inyu,cofiuy,inby,ny+1,0,nz1,nz1)  !buf2=dudz=[d(u_x)dz]_y                        
     !$OMP DO SCHEDULE(STATIC)
     do j = 1,ny
...
!----------------------------------------------------------
     ...
     call interpyy_2(v(0,1,i),buf3,inyv,cofivy,inby,ny,1,nz1,nz1) 	!(buf3=v_y)  
     call differzy_2(buf3,buf4,dcyv,dcby,cofcyv,1,1,0,1,ny)               !buf4=dvdy=d(v_y)/dy	          
#ifdef INFOINTER 
        !$OMP DO SCHEDULE(STATIC) 
        do j=1,ny          
        ...
        enddo
#endif     
     call differzy_2(w(0,1,i),buf3(0,1),dcyv,dcby,cofcyv,1,1,0,1,ny+1)    !buf3=dwdy !!JSS:corrected the coeff May 25 (now v, and 1,1,0,1)
...
!----------------------------------------------------------
...
#ifndef NO3PRODUCTS
     !=====================Triple products 
     call interpyy_2(u_x(0,1,i),buf2,inyu,cofiuy,inby,ny+1,0,nz1,nz1) 		  !buf2=[u_x]_y in Fourier (faster)         
     call fourxz_2(buf2(0:nz2,jbf1:jef1),bphy1(1:nz+2,jbf1:jef1),1,ny+1,jbf1,jef1)  !bphy1=[u_x]_y real              
     !$OMP BARRIER         !(Now: bphy1==buf1=wkp USED. U REAL)      
     call interpyy_2(w(0,1,i),buf2,inyu,cofiuy,inby,ny+1,0,nz1,nz1) 		 !w_y fourier in Fourier (faster)
     call fourxz_2(buf2(0:nz2,jbf1:jef1),bphy3(1:nz+2,jbf1:jef1),1,ny+1,jbf1,jef1) !bphy3=w_y real
     !$OMP BARRIER	   !(Now: bphy3==buf4=bufuphy USED. W REAL)         
     call fourxz_2(v(0:nz2,jbf2:jef2,i),bphy2(1:nz+2,jbf2:jef2),1,ny,jbf2,jef2) 	 !bphy2=v real 
  ...
  call differxx_2(upencil,dudx_pencil,dcxv,dcbx,cofcxv,2,mpu)  !dudx_pencil=d(u)dx pencils @th center of the cell
  call chx2p_2(dudx_pencil,dudx_zy,dwdx,mpiid,ny+1,communicator)            !dudx_zy =d(u)dx in planes . dwdx==buffer
  !==============================================================================
  ...
  do i=ib,ie
     ...
     enddo
     !call interpyy_2(buf1,buf1,inyu,cofiuy,inby,ny+1,0,nz1,nz1) !buf1=p_y  !works in place 
     !---------------------------------------------
     !Case for u: dudx_zy=dudx
        ...
        enddo
#endif                   
     call interpyy_2(dudx_zy(0,1,i),dudx_zy(0,1,i),inyu,cofiuy,inby,ny+1,0,nz1,nz1) !dudx_zy=dudx=(du/dx)_y           
     !$OMP DO SCHEDULE(STATIC)
     do j=1,ny
...
!===============CORRELATIONS==================
#ifndef NOCORR
call compute_corr_2(coru,corv,corw,coruv,corp,corox,coroy,coroz,buf_cor,buf_corp,dwdx,dudx_zy,mpiid,communicator)
#endif
end subroutine statsp_2
...
subroutine compute_corr_2(cor1,cor2,cor3,cor4,cor5,cor6,cor7,cor8,buf_cor,buf_corp,buf_change,buf_cor2,mpiid,communicator)
  use ctesp_2,only: nx,ny,nz,nz2,ncorr,lxcorr,xcorpoint,nxp,xci,xco
  use point_2
  use omp_lib
  implicit none
  include 'mpif.h'
  ...
  do ii=1,lxcorr
     do j=1,7       
        call chp2x_2(buf_corp(1,1,j,ii),buf_cor2(0,1,ib,j,ii),buf_change,mpiid,ncorr,communicator)       
     enddo
     !Computing correlations:     
     call c_corr_2(cor1 (1,1,ii),buf_corp(1,1,1,ii),buf_corp(1,1,1,ii),ii,mpiid)!cor_u   
     call c_corr_2(cor2 (1,1,ii),buf_corp(1,1,2,ii),buf_corp(1,1,2,ii),ii,mpiid)!cor_v   
     call c_corr_2(cor3 (1,1,ii),buf_corp(1,1,3,ii),buf_corp(1,1,3,ii),ii,mpiid)!cor_w   
     call c_corr_2(cor4 (1,1,ii),buf_corp(1,1,1,ii),buf_corp(1,1,2,ii),ii,mpiid)!cor_uv   
     call c_corr_2(cor5 (1,1,ii),buf_corp(1,1,4,ii),buf_corp(1,1,4,ii),ii,mpiid)!cor_p   
     call c_corr_2(cor6 (1,1,ii),buf_corp(1,1,5,ii),buf_corp(1,1,5,ii),ii,mpiid)!cor_ox   
     call c_corr_2(cor7 (1,1,ii),buf_corp(1,1,6,ii),buf_corp(1,1,6,ii),ii,mpiid)!cor_oy   
     call c_corr_2(cor8 (1,1,ii),buf_corp(1,1,7,ii),buf_corp(1,1,7,ii),ii,mpiid)!cor_oz   
  enddo
end subroutine compute_corr_2
...
subroutine c_corr_2(cor,buffer1,buffer2,j,rank)
  use ctesp_2,only: nx,ny,nz,nz2,lxcorr,xcorpoint,nxp,xci,xco
  use point_2
  implicit none
  include 'mpif.h'
  ...
  do pn=1,mp_corr2  !pencil=Each 2 buffers pencils=1 correlation pencil                     
     ...
     enddo
  enddo
end subroutine c_corr_2




! **********************  minimal txapuza  ************* 
subroutine ministats_2(uiy,viy,wiy,uner,hy,i)
  use point_2
  use ctesp_2
  implicit none
