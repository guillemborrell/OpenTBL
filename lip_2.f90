...
subroutine diffy_inplace_2(v_in,v_out,c)
  use ctesp_2
  use point_2
  implicit none
  include "mpif.h"
  ...
  do kk=0,2*nz2+1,blockl
     ...
     enddo
   enddo
 end subroutine diffy_inplace_2
...
subroutine diffx_inplace_2(u_in,u_out,c,nlin)
  use ctesp_2
  implicit none
  ...
  do k =1,nlin
     ...
     enddo
  enddo
end subroutine diffx_inplace_2



!Compute interpolation in X direction (mid-point interpolation)
!(does not work in place, use different buffers u_in,u_out)
subroutine interpx_new_2(u_in,u_out,c,nlin)
  use ctesp_2
  implicit none
  real*8 u_out(nx,nlin),u_in(nx,nlin),c(8,nx)
  integer i,j,k,l,nlin
  ...
  do k =1,nlin
     ...
     enddo
  enddo
end subroutine interpx_new_2


!Compute interpolation in Y direction (mid-point interpolation)
!(does not work in place, use different buffers u_in,u_out)
subroutine interpy_new_2(u_in,u_out,c,tipo)
  use ctesp_2
  use point_2
  implicit none
  include "mpif.h"
