...
subroutine comm_setup(NX,NY,NZ,rank,size,node,domain,pblock)
...
subroutine chp2xv(pen,plan,buf,size,communicator)
  ...
  call transpose1(plan,buf,nodev,domainv,size)
  ...
  call transpose2(buf(nodev%bound/2+1:nodev%size),pen,nodev,domainv)
...
subroutine chx2pv(plan,pen,buf,size,communicator)
  ...
  call transpose3(pen,buf(nodev%bound/2+1:nodev%size),nodev,domainv)
  ...
  call transpose4(buf,plan,nodev,domainv,size)
...
subroutine chp2xc(pen,plan,buf,size,communicator)
  ...
  call transpose1(plan,buf,node_corr,domain_corr,size)
  ...
  call transpose2(buf(node_corr%bound/2+1:node_corr%size),pen,node_corr,domain_corr)
...
subroutine chx2pc(plan,pen,buf,size,communicator)
  ...
  call transpose3(pen,buf(node_corr%bound/2+1:node_corr%size),node_corr,domain_corr)
  ...
  call transpose4(buf,plan,node_corr,domain_corr,size)
...
subroutine transpose1(var,buf,node,domain,size)
...
subroutine transpose4(buf,var,node,domain,size)
  use shared_mem
  implicit none
  type(domaindata), intent(in):: domain
  type(nodedata), intent(in):: node
...
end subroutine transpose4

subroutine transpose2(buf,var,node,domain)
...
subroutine transpose3(var,buf,node,domain)
  use shared_mem
  implicit none
  type(domaindata), intent(in):: domain
  type(nodedata), intent(in):: node
