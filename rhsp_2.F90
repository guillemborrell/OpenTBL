...
subroutine rhsp_2(ut,vt,wt,pt,rhsupat,rhsvpat,rhswpat, &
...
!   if (mpiid==mpiout) write(*,*) '**ut(0,250,xout)** 1',ut(0,250,xout)	  
  ...
  do i=ib,ie   ! ---- i=1 has to done in this case to diff. in x
     ...
     call fourxz_2(ut(0:nz2,jbf1:jef1,i),wkp(1:nz+2,jbf1:jef1),1,ny+1,jbf1,jef1)                !  u (phys)
     !$OMP END PARALLEL
     ...
     wmtmp=-1e14;vmtmp=-1e14  !firstprivate does not work in BG
        ...
        do j=jbf1,jef1
           call fourxz_2(wki1t(0,j,i),wki2r(1:nz+2,ompid),1,1,1,1)         
           wki2r(1:nz,ompid)    = wki2r(1:nz,ompid)**2                  ! u_x**2
           call fourxz_2(wki1t(0,j,i),wki2r(1:nz+2,ompid),-1,1,1,1)     !<<<<<<<<<<<<<<<<<<<  here !!!!
           call fourxz_2(wki3t(0,j,i),wki2r(1:nz+2,ompid),1,1,1,1)       
        ...
        call interpyy_2(wkp,wkpo,inyu,cofiuy,inby,ny+1,0,nz+2,nz) ! u interpolated in 'y'      
         !$OMP BARRIER    !extra barrier
        do j=jbf2,jef2                                      
           call fourxz_2(wki2t(0,j,i),wki2r(1:nz+2,ompid),1,1,1,1)		
     ...
     call differzy_2(wki2t(0,1,i),wkf,dcyu,dcby,cofcyu,3,1,-1,0,ny+1)
        !$OMP BARRIER    !extra barrier
  ...
  call chp2x_2(wki1,wki1t,rhswt,mpiid,ny+1,communicator)
  call difvisxx_2(wki1,resu,wki1,dcxu,vixu,dcbx,cofcxu,cofvxu,cofvbx,1,mpu,1,rex) 
  call chx2p_2(wki1,wki1t,rhswt,mpiid,ny+1,communicator)
  
  ! rhsv: wki2 contains uv, compute duv/dx. resv contains v in pencils, compute d2(v)/d2x
  call chp2x_2(wki2,wki2t,rhswt,mpiid,ny,communicator)
  call difvisxx_2(wki2,resv,wki2,dcxv,vixv,dcbx,cofcxv,cofvxv,cofvbx,2,mpv,0,rex)  
  call chx2p_2(wki2,wki2t,rhswt,mpiid,ny,communicator)

  ! rhsw: wki3 contains uw, compute duw/dx. resw contains w in pencils, compute d2(w)/d2x
  call chp2x_2(wki3,wki3t,rhswt,mpiid,ny+1,communicator)
  call difvisxx_2(wki3,resw,wki3,dcxv,vixv,dcbx,cofcxv,cofvxv,cofvbx,2,mpu,0,rex)
  call chx2p_2(wki3,wki3t,rhswt,mpiid,ny+1,communicator)
  ! =============================================================
...
!   if (mpiid==mpiout) write(*,*) '**ut(0,250,xout)** 2',ut(0,250,xout)	
  ...
  if(mpiid.eq.0) then
    write(*,*) 'RECEIVING THE INLET FROM BL1.............................' 
  call genflu_2(ut,vt,wt,y,re,dt,tiempo,mpiid,m,communicator) !This is not call anymore
!     call MPI_RECV(ut(0,1,1),(nz2+1)*(ny+1),MPI_REAL8,mpiids1(3+1),1,MPI_COMM_WORLD,istat,ierr)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i = ib0,ie  
    if(i.ne.nx) then !this is correct... rhsu(NX)=U_inf*dudx; rhsv(Nx)=U_inf*dvdx (this is done when in pencils)
     ...
     call interpyy_2(reswt(0,1,i),wkf,inyu,cofiuy,inby,ny+1,0,nz1,nz1)    
     !$OMP BARRIER    !extra barrier     
     call fourxz_2(wkf(0:nz2,jbf1:jef1),wkpo(1:nz+2,jbf1:jef1),1,ny+1,jbf1,jef1) ! wkpo = w interp in y     
     !$OMP BARRIER    !extra barrier     

     do j = jbf2,jef2                         !!!  what happens to ny+1 ??               
        call fourxz_2(resvt(0,j,i),wki2r(1:nz+2,ompid),1,1,1,1) 
        wkp(1:nz,j) = wki2r(1:nz,ompid)*wkpo(1:nz,j)        !vw at v locations           
     enddo
     !$OMP BARRIER

     call fourxz_2(wkfo(0:nz2,jbf1:jef1),wkp(1:nz+2,jbf1:jef1),-1,ny+1,jbf1,jef1)    ! wkfo = vw at v locations
     !$OMP BARRIER
     ...
     call differzy_2(wkfo,wkf,dcyu,dcby,cofcyu,3,1,-1,0,ny+1)    
     !$OMP BARRIER    !extra barrier          
     do j = jbf1,jef1            
     ...
     call interpyy_2(resvt(0,1,i),wkf,inyv,cofivy,inby,ny,1,nz1,nz1)
     !$OMP BARRIER    !extra barrier
     call fourxz_2(wkf(0:nz2,jbf2:jef2),wkpo(1:nz+2,jbf2:jef2),1,ny,jbf2,jef2)     !!!  v (phys)
     !$OMP BARRIER 
     ...
     call fourxz_2(wkfo(0:nz2,jbf2:jef2),wkp(1:nz+2,jbf2:jef2),-1,ny,jbf2,jef2)
     !$OMP BARRIER
     call differzy_2(wkfo,wkf,dcyv,dcby,cofcyv,1,1,0,1,ny)
     !$OMP BARRIER    !extra barrier    
     do j=jbf2,jef2
        rhsvt(0:nz2,j,i) = rhsvt(0:nz2,j,i) + wkf(0:nz2,j)	               
        ! ------- dww/dz for w-equation   (ny+1 is superseeded by bc)             
        call fourxz_2(reswt(0,j,i),wki2r(1:nz+2,ompid),1,1,1,1)       
        wki2r(1:nz,ompid) = wki2r(1:nz,ompid)**2 
        call fourxz_2(wkf(0,j),wki2r(1:nz+2,ompid),-1,1,1,1)                              
  ...
  ! -- final velocity updates (var4(m=1)=0)     
    do i=ib0,ie
      ...
      !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(j) 
      call vistyy_2(resvt(0,1,i),wkf,viyv,cofvyv,cofvby,ny)
      !$OMP DO SCHEDULE(STATIC)
	...
	wki2t(:,j,i) =  vt(:,j,i)-(var3*rhsvt(:,j,i)+var2*wkf(:,j)+var4*rhsvpat(:,j,i))  		
      ...
      call vistyy_2(resut(0,1,i),wkf,viyu,cofvyu,cofvby,ny+1)                         
      !$OMP DO SCHEDULE(STATIC)
	...
	wki1t(:,j,i) =   ut(:,j,i) -(var3*rhsut(:,j,i)+var2*wkf(:,j)+var4*rhsupat(:,j,i)) 		            
      ...
      call vistyy_2(reswt(0,1,i),wkf,viyu,cofvyu,cofvby,ny+1)
      !$OMP DO SCHEDULE(STATIC)
      do j=1,ny+1   !in the implicit terms, always from 1:jee-1=1:ny at least!!	
  ...
  rkk =rex*dt*rkdv(m)
  do i=ib0,ie
     call implzy_2(ut(0,1,i),wki1t(0,1,i),vyui,cofvyu,ny+1,rkk)
     call implzy_2(vt(0,1,i),wki2t(0,1,i),vyvi,cofvyv,ny  ,rkk)
...
!   if (mpiid==mpiout) write(*,*) '**ut(0,250,xout)** 4',ut(0,250,xout)
  ener(13:15)=0
  dostat  = .FALSE.  
end subroutine rhsp_2


! ===============================================================
subroutine energies_2(ut,vt,wt,hy,ener,communicator)
  use ctesp_2
  use point_2
  implicit none
  include 'mpif.h'
  ...
  uner=0d0
  do i=ib,ie
     call ministats_2(ut(0,1,i),vt(0,1,i),wt(0,1,i),uner,hy,i) 
  enddo
